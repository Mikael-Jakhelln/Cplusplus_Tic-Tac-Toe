...
namespace bot{
	...
	class bot{
		...
		vector<int> getcoords(int size, vector< vector<char> > board)
		{
			...
			if(playernumber == 1) //the bot is player 1 'X'
			{
				playerpiece = 'X';
				opponentpiece = 'O';
			}
			else	//the bot is player 2 'O'
			{
				playerpiece = 'O';
				opponentpiece = 'X';
			}
			...
			else if(difficulty == 2)
			{
				...
				if(checkempty(size, board))
					return randommove(size); //do random move if the board is empty, just for kicks, might change it when i bother to read tic-tac-toe strategy
				else
					return bettermove(size, board);
			}
			else if(difficulty > 2) //difficulties above 2 are not implemented yet. insert even better algorithm here..
			{
		...
		{	
			...
			{
				cout << "DEBUG: checrow couldnt find a move, maybe all rows are a draw" << endl;
			}
			vector<int> c(2); //returnvector
			c[0]=a[0];	//sets row
			c[1]=a[1];	//sets column
			cout << "DEBUG: bot.bettermove = " << c[0] << "," << c[1] << endl;
			return c;
		}
		...
		{ 
			...
			for(int x=0; x<n; x++) //loops trough all rows
			{
				...
				{
					if(gb[x][y] == playerpiece) //counts moves left untill win for that row
						movestowin--;
